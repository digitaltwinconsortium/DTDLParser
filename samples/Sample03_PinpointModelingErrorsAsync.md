...
This sample follows the same flow as [Fix an invalid DTDL model asynchronously](./Sample02_FixInvalidDtdlModelAsync.md) but illustrates use of the `DtdlParseLocator` to provide pinpoint accuracy to identified modeling errors.
A [synchronous version](./Sample03_PinpointModelingErrors.md) of this sample is also available.
...
```C# Snippet:DtdlParserSample03Async_CreateModelParser
...
```C# Snippet:DtdlParserSample03Async_ObtainInvalidDtdlText
string jsonText =
@"{
...
Without support from the calling code, the `ModelParser` reports the location of an error with reference to a DTDL element with an `@id` property, as illustrated in sample [Fix an invalid DTDL model asynchronously](Sample02_FixInvalidDtdlModelAsync.md).
...
```C# Snippet:DtdlParserSample03Async_CreateDtdlParseLocator
...
```C# Snippet:DtdlParserSample03Async_CallParseAsync
...
```C# Snippet:DtdlParserSample03Async_CallWait
try
{
    parseTask.Wait();
...
```C# Snippet:DtdlParserSample03Async_CorrectPropertyName
jsonText =
@"{
  ""@context"": ""dtmi:dtdl:context;3"",
  ""@id"": ""dtmi:example:anInterface;1"",
...
```C# Snippet:DtdlParserSample03Async_RepeatCallParseAsync
parseTask = modelParser.ParseAsync(jsonText, locator);
```

[repeat]: # (Snippet:DtdlParserSample03Async_CallWait)
...
```C# Snippet:DtdlParserSample03Async_CorrectTypeName
jsonText =
@"{
  ""@context"": ""dtmi:dtdl:context;3"",
  ""@id"": ""dtmi:example:anInterface;1"",
...
[repeat]: # (Snippet:DtdlParserSample03Async_RepeatCallParseAsync)
...
```C# Snippet:DtdlParserSample03Async_AddRequiredProperty
jsonText =
@"{
  ""@context"": ""dtmi:dtdl:context;3"",
  ""@id"": ""dtmi:example:anInterface;1"",
