            ...
            foreach (JsonLdProperty prop in elt.Properties)
            {
                switch (prop.Name)
                {
                    ...
                    case "dtmi:dtdl:property:target;4":
                        ...
                        {
                            targetProperty = prop;
                            Dtmi newTarget = (Dtmi)ValueParser.ParseSingularIdentifierValueCollection(aggregateContext, this.Id, "target", prop.Values, 2048, TargetPropertyRegexPatternV4, layer, parsingErrorCollection, isOptional: true, requireDtmi: true);
                            if (this.targetPropertyLayer != null)
                            {
                                if (this.Target != newTarget)
                                {
